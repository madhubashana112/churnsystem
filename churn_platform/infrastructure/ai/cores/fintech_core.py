...
# Beyond the shared prediction shape, FinTech distinguishes the kind of
# dormancy, which decides whether the account is recoverable.
SECTOR_FIELDS = ("dormancy_type",)
...
class FintechCore(IChurnCore):
    ...
    async def analyze(
        ...
        response = await self.gateway.generate_json(
            FINTECH_CORE_SYSTEM_PROMPT, build_user_prompt(features)
        )
