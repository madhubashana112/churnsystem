...
# Beyond the shared prediction shape, Telecom carries the diagnosed cause and
# whether the account sits in a region with a known network problem.
SECTOR_FIELDS = ("root_cause", "regional_network_impact_flag")
...
class TelecomCore(IChurnCore):
    ...
    async def analyze(
        ...
        response = await self.gateway.generate_json(
            TELECOM_CORE_SYSTEM_PROMPT, build_user_prompt(features)
        )
